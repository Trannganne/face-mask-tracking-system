...
\renewcommand{\arraystretch}{1.5} 
\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{5cm}|>{\raggedright\arraybackslash}p{3.5cm}|>{\raggedright\arraybackslash}X|}
\hline
\rowcolor{headerblue} 
...
% Tăng khoảng cách dòng cho bảng thoáng hơn
\renewcommand{\arraystretch}{1.5} 
...
RandomRotation & $\pm 15^\circ$ & Mô phỏng các trạng thái khuôn mặt nghiêng. \\ \hline
ColorJitter & brightness = 0.2, \newline contrast = 0.2, \newline saturation = 0.2 & Mô phỏng điều kiện ánh sáng khác nhau trong thực tế giám sát. \\ \hline
Normalize (ImageNet) & mean = [0.485, 0.456, 0.406], \newline std = [0.229, 0.224, 0.225] & Chuẩn hóa phân phối pixel về cùng thang đo giúp mô hình hội tụ nhanh hơn. \\ \hline
\end{tabularx}
\end{table}
...
% Giữ nguyên độ thoáng của các hàng (có thể bỏ nếu bạn đã khai báo 1 lần ở đầu tài liệu)
\renewcommand{\arraystretch}{1.5} 
...
\renewcommand{\arraystretch}{1.5} 
\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{3cm}|>{\raggedright\arraybackslash}X|>{\centering\arraybackslash}p{3.2cm}|>{\raggedright\arraybackslash}p{2.8cm}|}
\hline
\rowcolor{headerblue} 
...
Conv Block 1 & Conv2D ($3\rightarrow32$) + BN + ReLU + MaxPool(2) + Dropout(0.25) & $64 \times 64 \times 32$ & 32 filters \\ \hline
Conv Block 2 & Conv2D ($32\rightarrow64$) + BN + ReLU + MaxPool(2) + Dropout(0.25) & $32 \times 32 \times 64$ & 64 filters \\ \hline
Conv Block 3 & Conv2D ($64\rightarrow128$) + BN + ReLU + MaxPool(2) + Dropout(0.25) & $16 \times 16 \times 128$ & 128 filters \\ \hline
Conv Block 4 & Conv2D ($128\rightarrow192$) + BN + ReLU + MaxPool(2) + Dropout(0.3) & $8 \times 8 \times 192$ & 192 filters \\ \hline
Global Average Pooling & AdaptiveAvgPool2d(1,1) & $1 \times 1 \times 192$ & - \\ \hline
Fully Connected & Flatten + Linear($192\rightarrow128$) + BN + ReLU + Dropout(0.5) \newline Linear($128\rightarrow64$) + BN + ReLU + Dropout(0.4) \newline Linear($64\rightarrow3$) & 3 & 3 classes \\ \hline
...
% Tăng khoảng cách dòng để bảng thoáng và dễ đọc hơn
\renewcommand{\arraystretch}{1.5} 
...
\bibitem{1}
Mohammadpour, M., et al. (2022). A Survey of Deep Learning Techniques for Object Tracking.

\bibitem{2}
FPT AI. (2026). CCNN Documentation.
