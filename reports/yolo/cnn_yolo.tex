...
\title{Tìm hiểu: \\ Mạng Nơ-ron tích chập (CNN)\\ và \\cài đặt, triển khai Yolo11}
\author{}
...
Mạng Nơ-ron Tích chập (Convolutional Neural Network - CNN) là công nghệ cốt lõi trong lĩnh vực thị giác máy tính (Computer Vision), được ứng dụng rộng rãi trong nhận diện khuôn mặt, xe tự lái và xử lý hình ảnh.
...
\begin{itemize}
    ...
    \item Việc này làm mất đi toàn bộ thông tin về cấu trúc không gian (các điểm ảnh/pixel nằm kề nhau).
    \item Với ảnh có độ phân giải cao, số lượng tham số của mạng MLP quá to, dẫn đến việc huấn luyện cực kỳ khó khăn.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Khái niệm:} Đây là lớp quan trọng nhất, sử dụng các bộ lọc (Kernel) trượt dọc qua toàn bộ bức ảnh.
    \item \textbf{Chức năng:} Mỗi bộ lọc đóng vai trò như một kính lúp chuyên tìm kiếm một đặc trưng (cạnh ngang, góc cong, đường chéo).
